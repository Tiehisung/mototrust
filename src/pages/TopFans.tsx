// pages/TopFans.tsx - Modified version using real data
import {
...
export default function TopFansPage() {
  ...
  const showRegisterButton = user && !user.isFan;
  ...
  const totalPoints = mockFans.reduce((sum, fan) => sum + fan.fanPoints, 0);
  const averageEngagement = Math.round(
    mockFans.reduce((sum, fan) => sum + fan.engagementScore, 0) / totalFans,
  );

  return (
    <>
      <Helmet>
        <title>Top Fans | {TEAM.name}</title>
        <meta
          name="description"
          content={`Meet the most dedicated ${TEAM.name} fans. See who's leading the fan rankings and engagement scores.`}
        />
        <meta property="og:title" content={`Top Fans - ${TEAM.name}`} />
        <meta
          property="og:description"
          content="Celebrating our most passionate supporters."
        />
      </Helmet>
